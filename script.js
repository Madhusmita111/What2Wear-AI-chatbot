...
async function getBotResponse(message) {
    if (!userPreferences.name) {
        ...
        return `Hi ${message}! I'm your AI Outfit Assistant. I'd love to help you create the perfect outfit. What's your preferred style? (Casual, Professional, Ethnic, Indo-Western, etc.)`;
    }
    ...
    try {
        const prompt = {
            contents: [{
                parts: [{
                    text: `You are the AI Outfit Assistant. Create a detailed outfit suggestion for ${userPreferences.name} using this exact format:

OCCASION: ${request.occasion}
DAY: ${request.timing}

MAIN OUTFIT
[Provide 3-4 main clothing items. For each item, specify:
...
Requirements:
1. Style Preference: ${userPreferences.style}
2. Occasion Type: ${request.occasion}
3. Day/Timing: ${request.timing}
4. Make suggestions elegant and detailed
5. Include specific colors, materials, and styling
6. Consider the occasion's formality
7. Ensure all pieces coordinate well together

Create a unique, personalized outfit suggestion that matches ${userPreferences.name}'s ${userPreferences.style} style preference.`
                }]
            }]
        };
...
function analyzeRequest(message) {
    ...
    const occasions = {
        ...
        party: ['party', 'club', 'night out', 'dinner'],
